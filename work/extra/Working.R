...
con <- dbConnect(drv, dbname = "r_words",
                 host = "localhost", port = 5432,
                 user = "ilyaperepelitsa", password = pw)
# rm(pw)
...
df_postgres <- as.data.frame(df_postgres)
summary(df_postgres)
...
df_postgres_small <- df_postgres %>% filter(documentdf <= 25)
